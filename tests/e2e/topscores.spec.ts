...
test('topscores: peers with last_seen_ts older than 7 days get the stale class', async ({
  page,
}) => {
  await page.goto('/?devtools=1');
  await waitForGameReady(page);
  ...
  await seedAndRender(
    page,
    {
      ...
      'sleeper@test': mkPeer({
        ...
        cash: 50,
        last_seen_ts: now - SEVEN_DAYS_MS - 1000, // just over the threshold
      }),
    },
    SELF,
  );

  const sleeperRow = page
    .locator('[data-testid="dd-leaderboard-row-sleeper@test"]')
    .first();
  await expect(sleeperRow).toHaveClass(/dd-leaderboard-row--stale/);

  const aliceRow = page
    .locator('[data-testid="dd-leaderboard-row-alice@local"]')
    .first();
  await expect(aliceRow).not.toHaveClass(/dd-leaderboard-row--stale/);
});
