...
// Unit tests for scripts/state.js
...
const { freshState, applyDelta, SCHEMA_VERSION } = require('../scripts/state');
...
describe('freshState', () => {
  ...
  it('seeds game_values from data/default_game.json', () => {
    ...
    // Matches the values in data/default_game.json
...
// ---------------------------------------------------------------------------
// A. Convergence: two divergent replay sequences with the same delta-set
...
describe('applyDelta — convergence (acceptance criterion A)', () => {
  ...
  it('reordering stub ops produces identical state', () => {
    ...
    // Sequence A: chronological order
    const stateA = replay([d1, d2, d3], addr);
    // Sequence B: divergent intermediate ordering, same final delta-set
    const stateB = replay([d2, d1, d3], addr);
    // Sequence C: another divergent ordering
  ...
  it('same delta-set applied to two independent starting states converges', () => {
    ...
    // Two "instances" — both start from freshState but replay independently
...
// ---------------------------------------------------------------------------
// B. Reset op wipes prior state (acceptance criterion B)
// ---------------------------------------------------------------------------
...
describe('applyDelta — reset op (acceptance criterion B)', () => {
  ...
  it('clears nodes and missions after reset', () => {
    // Manually inject state with data to ensure something is being wiped
    ...
    const resetDelta = makeDelta(addr, 'reset', 5000);
    const after = applyDelta(s, resetDelta);
  ...
  it('restores game_values to seed defaults after reset', () => {
    let s = freshState(addr);
    // Simulate accumulated economy values
    s = { ...s, game_values: { ...s.game_values, cash_value: 99999, xp_level: 10 } };

    const resetDelta = makeDelta(addr, 'reset', 5000);
    const after = applyDelta(s, resetDelta);

    // Back to default_game.json seed values
...
// ---------------------------------------------------------------------------
// C. Schema-version mismatch handled gracefully (acceptance criterion C)
// ---------------------------------------------------------------------------
...
describe('applyDelta — schema-version mismatch (acceptance criterion C)', () => {
  ...
  it('resets to valid fresh state when schema_version is a future value', () => {
    ...
    const delta = makeDelta(addr, 'buyKarma', 1000);
    const result = applyDelta(futureState, delta);
...
describe('applyDelta — other-peer filter', () => {
  it('ignores deltas whose addr differs from state.addr', () => {
    ...
    // State object reference unchanged — no mutation at all
  ...
  it('processes own-addr deltas normally', () => {
    const addr = 'alice@example.com';
    const s = freshState(addr);
    // reset is the only op with visible state change
